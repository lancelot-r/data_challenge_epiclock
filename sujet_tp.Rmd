---
title: "Chronological age Prediction Challenge 1.1 (epiclock1.1)"
...
https://codabench.univ-grenoble-alpes.fr/competitions/92/?secret_key=ec6d2a3e-5e7f-4e73-be3b-f3e7385db84a

L’**objectif** est double : 

1. on cherche à expliquer l’age des individus dans le jeu de données `data_train` ;
2. pour prédire l’age des individus dans le jeu de données `data_test`.
...
**Distribution du méthylome dans `data_train`**

```{r distr_data_rain, echo=TRUE, results="verbatim"}
layout(matrix(1:2, 1), respect=TRUE)
plot(density(as.matrix(data_train[,3:10002])), main="Methylome")
```

Nous considérerons que la distribution de la valeur de chaque sonde est **gaussienne**.


# Méthode SIS

La method SIS [Shurely Independant Screening, Zhang HH. J R Stat Soc Series B Stat Methodol. 2008] appliquée au *méthylome* (définir) consiste à 
i) réaliser autant de regressions linéaires simples du type $age \sim probe$ qu’il y a de sondes ; 
ii) selectionner les sondes correspondant aux meilleurs modèles ($probe_1, probe_2, probe_3 ...$) ; 
...
paste0(sis_probes[1:8], collapse="+")
```
...
r2_sis = c()
for (i in 0:300) {
 m = model_sis_i(data_train, i, screening_func=msiscreening) 
...
```{r}
set.seed(1)
data_train1 = data_train[sample(round(nrow(data_train) / 2)),]
dim(data_train1)
...
rmse_train2 = c()
for (i in 0:200) {
 m = model_sis_i(data_train1, i, screening_func=msiscreening) 
 r2_sis = c(r2_sis, summary(m)$r.squared)
 rmse_train1 = c(rmse_train1, sqrt(sum((data_train1$age - predict(m, data_train1))^2 / nrow(data_train1))))
 rmse_train2 = c(rmse_train2, sqrt(sum((data_train2$age - predict(m, data_train2))^2 / nrow(data_train2))))
}
layout(matrix(1:2, 1), respect=TRUE)
plot(0:200, r2_sis)
plot(0:200, rmse_train1, col=2)
points(0:200, rmse_train2, col=4)
legend("topright", col=c(2.4), c("data_train1", "data_train2"), pch=1)
...
Nous venons de voir que tous les sondes n’apportent pas la même quantité d’information "nouvelle". Nous allons tirer partie de la méthode de sélection variables *step forward* pour sélectionner les sondes qui apportent de l’information "nouvelle". En effet, nous allons partir du modèle nul et ajouter un à un les sondes, parmi les 50 meilleurs gênes obtenu grace à la méthode SIS, qui augmentent considérablement la qualité du modèle.
